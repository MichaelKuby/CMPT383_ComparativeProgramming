...
pub fn quicksort_partial<T : Copy + Ord>(v: &mut Vec<T>, left: usize, right: usize) {
    ...
    if right > left {
        // For lengths below INSERTION_SORT_CUTTOFF, call insertion_sort instead of recursing
        if (right - left) < INSERTION_SORT_CUTOFF {
            insertion_sort(v, left, right)
        }
        else {
            // Otherwise, pick a value, called a pivot, that occurs in the range, and partition
            // the range (reorder its elements
            let pivot : usize = partition(v, left, right);
...
pub fn quicksort<T: Copy + Ord>(v: &mut Vec<T>) {
    let left : usize = 0;
    let right : usize = v.len();
    quicksort_partial(v, left, right-1);
}